...
def build_payload(inputs: dict[str, Path], parameters: dict | None = None, tools: dict | None = None) -> dict:
    return {
...
def write_manifest(path: Path, payload: dict, outputs: list[Path]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    data = {**payload, "signature": signature(payload), "outputs": [str(item) for item in outputs]}
