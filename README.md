...
- `work/foldx/<enzyme>/individual_list.txt`：FoldX 突变列表；
- `work/foldx/<enzyme>/status.jsonl`：运行状态；
- `results/<enzyme>/foldx_results.csv`：逐突变 ΔΔG；
...
GitHub Actions 会在 Python 3.10、3.11 和 3.12 上运行。单元测试覆盖在线结果标准化、FASTA—PDB 比对、FoldX 突变语法、`Dif_*.fxout` 解析和报告逻辑。真实 FoldX 执行仍需本机安装 FoldX。
